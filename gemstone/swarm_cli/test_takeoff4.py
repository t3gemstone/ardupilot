...
conn.mav.param_set_send(1, 1, b"Q_GUIDED_MODE", 1.0, mavutil.mavlink.MAV_PARAM_TYPE_REAL32)
time.sleep(1)
...
print("Sending MAV_CMD_NAV_TAKEOFF (22)")
conn.mav.command_long_send(1, 1, mavutil.mavlink.MAV_CMD_NAV_TAKEOFF, 0, 0, 0, 0, 0, 0, 0, 100)

start = time.time()
while time.time() - start < 5:
    msg = conn.recv_match(type=['GLOBAL_POSITION_INT', 'COMMAND_ACK'], blocking=False)
    if msg:
        if msg.get_type() == 'COMMAND_ACK' and msg.command == mavutil.mavlink.MAV_CMD_NAV_TAKEOFF:
            print(f"ACK for TAKEOFF: {msg.result}")
