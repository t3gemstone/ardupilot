...
void Compass::_probe_external_i2c_compasses(void)
{
    ...
    // external i2c bus
    FOREACH_I2C_EXTERNAL(i) {
        ADD_BACKEND(DRIVER_MMC5603,
                    AP_Compass_MMC5603::probe(GET_I2C_DEVICE(i, HAL_COMPASS_MMC5603_I2C_ADDR), true, ROTATION_NONE));
    }
#if !defined(HAL_SKIP_AUTO_INTERNAL_I2C_PROBE)
    FOREACH_I2C_INTERNAL(i) {
        ADD_BACKEND(DRIVER_MMC5603,
                    AP_Compass_MMC5603::probe(GET_I2C_DEVICE(i, HAL_COMPASS_MMC5603_I2C_ADDR), all_external, ROTATION_NONE));
    }
#endif
