...
void RCOutput_Sysfs::init()
{
    for (uint8_t i = 0; i < _channel_count; i++) {
        ...
        // pwmchip3, pwmchip5 and pwmchip7 has two channels each.
        // But channels of a pwmchip cannot differ in period value.
        if (i == 3) {
            // pwmchip3/pwm0
            _pwm_channels[i] = NEW_NOTHROW PWM_Sysfs(_chip+3, 0);
        } else if (i == 4) {
            // pwmchip7/pwm0
            _pwm_channels[i] = NEW_NOTHROW PWM_Sysfs(_chip+7, 0);
        } else if (i == 5 || i == 6) {
            // pwmchip5/pwm0, pwmchip5/pwm1
            _pwm_channels[i] = NEW_NOTHROW PWM_Sysfs(_chip+5, i - 5);
        } else {
