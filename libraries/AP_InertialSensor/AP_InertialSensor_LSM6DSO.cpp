...
#   define LSM6DSO_REG_FIFO_CTRL1_WTM_0   (0 << 0)
...
bool AP_InertialSensor_LSM6DSO::_hardware_init()
{
    ...
    if (whoami != WHO_AM_I) {
        DEV_PRINTF("LSM6DSO: unexpected WHOAMI 0x%02x (expected 0x%02x)\n", whoami, WHO_AM_I);
        return false;
    }
...
{
    // Scale for 2000DPS range (32768 counts per full scale)
    _gyro_scale = (2000.0f * DEG_TO_RAD) / 32768.0f;
}

void AP_InertialSensor_LSM6DSO::_set_accel_scale()
{
    // Scale for 8G range (32768 counts per full scale)
...
void AP_InertialSensor_LSM6DSO::_fifo_init()
{
    ...
    _register_write(LSM6DSO_REG_FIFO_CTRL1, LSM6DSO_REG_FIFO_CTRL1_WTM_0);
...
void AP_InertialSensor_LSM6DSO::_poll_data()
{
    ...
    uint16_t unread_sample_count = fifo_status[0] | ((fifo_status[1] & LSM6DSO_REG_FIFO_STATUS2_DIFF_MASK) << 8);

    // Process samples in batches to avoid excessive memory allocation
    while (unread_sample_count > 0) {
        uint16_t samples_to_read = MIN(unread_sample_count, FIFO_BATCH_SIZE);
        uint32_t buffer_size = samples_to_read * FIFO_SAMPLE_BYTES;

        uint8_t fifo_buffer[buffer_size];

        if (!_block_read(LSM6DSO_REG_FIFO_DATA_OUT_TAG, fifo_buffer, buffer_size)) {
            return;
        }

        for (uint16_t i = 0; i < samples_to_read; i++) {
            uint32_t offset = i * FIFO_SAMPLE_BYTES;

            uint8_t tag = fifo_buffer[offset] & LSM6DSO_REG_FIFO_DATA_OUT_TAG_MASK;

            switch (tag) {
            case LSM6DSO_REG_FIFO_DATA_OUT_TAG_G:
                _read_gyro_data(&fifo_buffer[offset + 1]);
                break;
            case LSM6DSO_REG_FIFO_DATA_OUT_TAG_XL:
                _read_accel_data(&fifo_buffer[offset + 1]);
                break;
            default:
                // Unknown tag - possibly temperature or timestamp data
                break;
            }
        }

        unread_sample_count -= samples_to_read;
    }
}
...
{
    Vector3f gyro(
        (int16_t)((gyro_data[1] << 8) | gyro_data[0]),  // X
...
{
    Vector3f accel(
        (int16_t)((accel_data[1] << 8) | accel_data[0]),  // X
